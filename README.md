...
- 使用浏览器页内搜索代码片段，快捷键：<kbd>Ctrl</kbd> + <kbd>F</kbd> or <kbd>command</kbd> + <kbd>F</kbd>
- 欢迎代码贡献者，请先阅读贡献指南(CONTRIBUTING.md).
- 代码片段使用`ES6`语法书写，使用[Babel transpiler](https://babeljs.io/)确保代码向后兼容.
- 您可以使用在这个[Repo](https://github.com/Rob-Rychs/30-seconds-of-code-texteditorsnippets)找到的代码片段文件，直接导入到您选择的(Vscode, Atom, Sublime)文本编辑器中.
- 您也可以直接导入这些代码片段到`Alfred 3`, 利用这个文件 [Repo](https://github.com/lslvxy/30-seconds-of-code-alfredsnippets).
...
⚠️ **WARNING:** 代码片段还没准备好在生成环境中使用.

你可以在[npm](https://www.npmjs.com/package/30-seconds-of-code)中找到包含所有代码片段的库.
...
> 重要提示: 使用完整版本连接和需要的目标版本替换`src`文件（例如：ES5 minified).
...
创建一个最多接收`n`个参数的函数，并且忽略其它参数.

使用展开运算符(`...`)和`Array.slice(0,n)`处理Arguments，得到不超过`n`个参数的数组，最后传递给回调函数`fn`调用.
...
给定一个键(`key`)和一组(`arguments`)，在给定一个上下文时(`contenxt`)调用函数. 主要用于函数的组合.

利用闭包来调用存储的键(`key`)跟(`arguments`).
...
将只接收数组参数的函数，改为接收可变参数.

给定一个函数，会返回一个闭包，它将收集输入的所有参数并转换为数组传入到原函数(`fn`).
...
Flip 将一个函数作为参数, 然后将第一个参数作为最后一个参数.

返回一个接收可变参数的闭包，并且在应用(`rest`)之前拼接最后一个参数使其成为一个参数.
...
创建一个函数，该函数使用它接收到的参数挨个调用，初始化提供的函数(`provided function`)并返回结果.

使用 `Array.map()` 和 `Function.apply()` 将每个函数应用于给定的参数.
...
函数的返回可以是以下的类型: 单个值, `Promise` 对象, 或者它们可以定义为 `async` , 通过 `await` 返回.
...
执行从左到右 (`left-to-right`) 的函数组合.

利用展开运算符 (`...`) 和 `Array.reduce()` 将函数从左到右 (`left-to-right`) 进行组合.

第一个函数(最左边)可以接收一个或者多个参数, 其余函数必须只能是一元函数(单参数函数).
